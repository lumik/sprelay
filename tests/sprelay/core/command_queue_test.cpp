...
void CommandQueueTest::uniquePush()
{
    ...
    QVERIFY(command_queue.get(k8090::CommandID::RELAY_ON).empty());

    // push command
    const int priority1 = 1;
    Command cmd1{k8090::CommandID::RELAY_ON, priority1, 1, 2, 3};
    command_queue.push(cmd1);
    QCOMPARE(command_queue.size(), std::size_t{1});
    QCOMPARE(*command_queue.get(k8090::CommandID::RELAY_ON)[0], cmd1);
    ...
    Command cmd2{k8090::CommandID::RELAY_OFF, priority2, 2, 3, 4};
    command_queue.push(cmd2);
    QCOMPARE(command_queue.size(), std::size_t{2});
    QCOMPARE(*command_queue.get(k8090::CommandID::RELAY_OFF)[0], cmd2);
    QCOMPARE(command_queue.stampCounter(), 2u);

    // push next command with the same priority
    Command cmd3{k8090::CommandID::TOGGLE_RELAY, priority2, 3, 4, 5};
    ...
    Command cmd4{k8090::CommandID::RELAY_ON, priority2, 255, 2, 3};
    command_queue.push(cmd4);
    QCOMPARE(command_queue.size(), std::size_t{3});
    QCOMPARE(*command_queue.get(k8090::CommandID::RELAY_OFF)[0], cmd2);
    QCOMPARE(*command_queue.get(k8090::CommandID::RELAY_ON)[0], cmd4);
    QCOMPARE(command_queue.stampCounter(), 3u);
    ...
    Command cmd5{k8090::CommandID::RELAY_ON, priority3, 255, 2, 3};
...
void CommandQueueTest::notUniquePush()
{
    ...
    const int priority1 = 1;
    Command cmd1{k8090::CommandID::RELAY_ON, priority1, 1, 2, 3};
    Command cmd2{k8090::CommandID::RELAY_ON, priority1, 2, 3, 4};
    command_queue.push(cmd1, false);
    command_queue.push(cmd2, false);
    QCOMPARE(command_queue.size(), std::size_t{2});
    QCOMPARE(command_queue.get(k8090::CommandID::RELAY_ON).size(), 2);
    QCOMPARE(*command_queue.get(k8090::CommandID::RELAY_ON)[0], cmd1);
    QCOMPARE(*command_queue.get(k8090::CommandID::RELAY_ON)[1], cmd2);
    QCOMPARE(command_queue.stampCounter(), 2u);

    // first unique and second not
    Command cmd3{k8090::CommandID::RELAY_OFF, priority1, 1, 2, 3};
    Command cmd4{k8090::CommandID::RELAY_OFF, priority1, 2, 3, 4};
    command_queue.push(cmd3, true);
    command_queue.push(cmd4, false);
    QCOMPARE(command_queue.size(), std::size_t{4});
    QCOMPARE(command_queue.get(k8090::CommandID::RELAY_OFF).size(), 2);
    QCOMPARE(*command_queue.get(k8090::CommandID::RELAY_OFF)[0], cmd3);
    QCOMPARE(*command_queue.get(k8090::CommandID::RELAY_OFF)[1], cmd4);
    QCOMPARE(command_queue.stampCounter(), 4u);

    // overwrite not unique by one unique
    Command cmd5{k8090::CommandID::RELAY_ON, priority1, 3, 4, 5};
    command_queue.push(cmd5, true);
    QCOMPARE(command_queue.size(), std::size_t{3});
    QCOMPARE(command_queue.get(k8090::CommandID::RELAY_ON).size(), 1);
    QCOMPARE(*command_queue.get(k8090::CommandID::RELAY_ON)[0], cmd5);
    QCOMPARE(command_queue.stampCounter(), 4u);
    // test if the time stamp was preserved
    QCOMPARE(command_queue.pop(), cmd5);
    QCOMPARE(command_queue.get(k8090::CommandID::RELAY_ON).size(), 0);
    QCOMPARE(command_queue.size(), std::size_t{2});
    QCOMPARE(command_queue.stampCounter(), 4u);
...
void CommandQueueTest::updateCommand()
{
    ...
    command_queue.push(cmd2, false);
    const int priority2 = 2;
    Command cmd3{k8090::CommandID::RELAY_ON, priority2, 3, 4, 5};
    command_queue.push(cmd3, false);
    Command cmd4{k8090::CommandID::RELAY_ON, priority1, 4, 5, 6};
    command_queue.push(cmd4, false);
    QCOMPARE(command_queue.size(), std::size_t{4});
    const QList<const Command *> & command_list1 = command_queue.get(k8090::CommandID::RELAY_ON);
    ...
    // update element
    const int priority3 = 3;
    cmd2 = Command{k8090::CommandID::RELAY_ON, priority3, 5, 6, 7};
    command_queue.updateCommand(1, cmd2);

    // pop elements
    QCOMPARE(command_queue.pop(), cmd2);
    QCOMPARE(command_queue.get(k8090::CommandID::RELAY_ON).size(), 3);
    QCOMPARE(command_queue.size(), std::size_t{3});
    QCOMPARE(command_queue.pop(), cmd3);
    QCOMPARE(command_queue.get(k8090::CommandID::RELAY_ON).size(), 2);
    QCOMPARE(command_queue.size(), std::size_t{2});
