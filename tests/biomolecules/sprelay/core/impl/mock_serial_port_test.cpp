...
void MockSerialPortTest::commandBenchmark_data()
{
    ...
    //  STX   CMD   MASK  PAR1  PAR2  CHK   ETX
        0x04, 0x21, 0x18, 0xdf, 0x70, 0x74, 0x0f  // set relay 4 and 5 to momentary and 5, 6 and 7 to timed and all
                                                  // except for 6 to toggle =>
                                                  //     momentary: 4, 5
                                                  //     toggle: 1, 2, 3, 7, 8
                                                  //     timed: 6
    };
...
void MockSerialPortTest::commandBenchmark()
{
    ...
    QVERIFY2(compareResponse(buffer, response),
        qPrintable(QString{"The response '%1' does not match the expected %2."}
            .arg(serial_utils::byte_to_hex(buffer, 7)).arg(serial_utils::byte_to_hex(response, 7))));
}
...
void MockSerialPortTest::jumperStatus()
{
    ...
    QVERIFY2(buffer[1] == jumper_off[1],
        qPrintable(QString{"The response '%1' does not match the expected %2."}
            .arg(serial_utils::byte_to_hex(&buffer[1], 1)).arg(serial_utils::byte_to_hex(&jumper_off[1], 1))));
    if (buffer[3]) {
...
void MockSerialPortTest::firmwareVersion()
{
    ...
    QVERIFY2(buffer[1] == version[1],
        qPrintable(QString{"The response '%1' does not match the expected %2."}
            .arg(serial_utils::byte_to_hex(&buffer[1], 1)).arg(serial_utils::byte_to_hex(&version[1], 1))));
    ...
    while (!remaining_responses.empty() && timer.isActive()) {
        ...
        for (int i = 0; i < n; i += 7) {
            ...
            QVERIFY2(it != remaining_responses.end(),
                qPrintable(QString{"The response '%1' does not match."}
                    .arg(serial_utils::byte_to_hex(&buffer[i], 7))));
...
void MockSerialPortTest::setMoreTimers()
{
    ...
    static const unsigned char responses[] = {
    //  STX   CMD   MASK  PAR1  PAR2  CHK   ETX
        0x04, 0x44, 0x01, 0x00, 0x01, 0xb6, 0x0f,
        0x04, 0x44, 0x02, 0x00, 0x01, 0xb5, 0x0f,
        0x04, 0x44, 0x04, 0x00, 0x01, 0xb3, 0x0f,
        0x04, 0x44, 0x08, 0x00, 0x01, 0xaf, 0x0f,
        0x04, 0x44, 0x10, 0x00, 0x02, 0xa6, 0x0f,
        0x04, 0x44, 0x20, 0x00, 0x02, 0x96, 0x0f,
        0x04, 0x44, 0x40, 0x00, 0x03, 0x75, 0x0f,
